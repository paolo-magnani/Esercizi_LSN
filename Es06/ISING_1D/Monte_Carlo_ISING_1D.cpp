...
//Read input informations
  ...
  if(restart==false){												//check if equilibration is already been done
		ifstream input("seed.in");
		input >> seed[0] >> seed[1] >> seed[2] >> seed[3];
		rnd.SetRandom(seed,p1,p2);
		input.close();
		//equil = true; 													//set to do the equilibration
   }else{
   	ifstream input("seed.out");
   	if(!input)	cerr << "Error opening file 'seed.out'";
   	input >> seed[0] >> seed[1] >> seed[2] >> seed[3];
		rnd.SetRandom(seed,p1,p2);
		input.close();  
		//equil = false;													//no need to do the equilibration
   }
...
void Move(int metro)
{
  ...
  //Select randomly a particle (for C++ syntax, 0 <= o <= nspin-1)
    ...
    else //Gibbs sampling
    {
		...
		double deltaE = energy_up - energy_down;
...
//cycle over spins
  ...
  walker[iu] = u; //raccolgo i dati nel walker 
  walker[ic] = u*u;
  walker[im] = m;
  walker[ix] = m*m;
}
...
void ConfFinal(void)
{
  ...
  if(restart==false) cout << "Equilibration concluded. You may restart the code with restart=1 to perform the simulation using the final spin configuration." << endl;
}

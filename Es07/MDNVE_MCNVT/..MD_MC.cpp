...
//Print initial values for measured properties
  ...
  cout << "Initial total energy     = " << walker[ie]/(double)npart << endl;
	cout << "Initial pressure			= " << walker[ip] << endl; //mio
  return;
}
...
// distance i-j in pbc
  ...
  walker[ie] = 4.0 * v + kin;  // Total energy;
  	walker[ip] = rho*walker[it] + (16./vol)*w/double(npart); //PRESSURE 
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, Acce;
   const int wd=12;
    ...
    stima_pot = blk_av[iv]/blk_norm/(double)npart + ((8.*M_PI*rho)*((1./9.)*pow(1./rcut, 9.)-((1./3.)*pow(1./rcut, 3.)))); //Potential energy
    ...
    stima_etot = blk_av[ie]/blk_norm/(double)npart + ((8.*M_PI*rho)*((1./9.)*pow(1./rcut, 9.)-((1./3.)*pow(1./rcut, 3.)))); //Total energy
    ...
    stima_pres = blk_av[ip]/blk_norm + ((32.*M_PI*rho)*((1./9.)*pow(1./rcut, 9.)-((1./6.)*pow(1./rcut, 3.)))); //PRESSURE 
...
//Acceptance
	 Acce << setw(wd) << iblk <<  setw(wd) << ACC/(iblk*attempted) << endl;
